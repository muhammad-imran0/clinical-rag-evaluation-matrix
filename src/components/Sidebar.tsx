    ...
    <>
      ...
      <header className="lg:hidden bg-slate-900 border-b border-slate-800 h-16 px-4 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold text-white tracking-tight">Dissertation Tracker</h1>
            <p className="text-xs text-slate-400">Muhammad Imran · UEL</p>
          </div>
        </div>
        <button 
      ...
      {/* Sidebar Container */}
      <aside className={`
        fixed inset-y-0 left-0 z-40 w-64 bg-slate-950 border-r border-slate-900 flex flex-col justify-between p-5 transition-transform duration-300 ease-in-out lg:static lg:translate-x-0
      ...
      `}>
        ...
        <div className="flex flex-col gap-6">
          <div className="hidden lg:flex items-center gap-3 pb-6 border-b border-slate-900">
            ...
            <div>
              <h1 className="text-lg font-bold text-slate-100 tracking-tight">MSc AI Dissertation</h1>
              <p className="text-sm text-slate-500 font-medium font-mono">UEL Portfolio Tracker</p>
            </div>
          </div>
          ...
          <div className="bg-slate-900/60 rounded-xl p-4 border border-slate-900 hover:border-slate-800 transition-colors">
            <div className="flex items-start gap-3">
              ...
              <div>
                <span className="text-xs uppercase tracking-wider text-slate-500 font-extrabold font-mono">Researcher</span>
                <h3 className="font-semibold text-slate-200 text-base">Muhammad Imran</h3>
                <code className="text-sm text-sky-400 mt-0.5 block font-mono">u3004795 · UEL</code>
              </div>
            </div>
            <div className="mt-3 pt-3 border-t border-slate-900/50 flex gap-2 items-center text-slate-400 text-sm">
          ...
          <nav className="flex flex-col gap-1">
            <span className="text-xs font-bold text-slate-600 uppercase tracking-wider px-3 mb-2 block font-mono">Navigation</span>
            {menuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  id={`nav-btn-${item.id}`}
                  className={`
                    w-full flex items-center gap-3 px-4 py-3 rounded-lg text-base transition-all duration-200 cursor-pointer
                    ${isActive 
        ...
        {/* Footer Block */}
        <div className="pt-6 border-t border-slate-900 text-sm text-slate-500 font-mono">
