...
export default function Sidebar({ currentView, onViewChange, simpleMode = false }: SidebarProps) {
  ...
  return (
    <>
      ...
      <header className={`lg:hidden border-b h-16 px-4 flex items-center justify-between sticky top-0 z-50 ${simpleMode ? 'bg-[#FAF7F0] border-[#E6DEC9]' : 'bg-slate-900 border-slate-800'}`}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-stone-500">Muhammad Imran · UEL</p>
          </div>
        </div>
        <button 
      ...
      {/* Sidebar Container */}
      <aside className={`
        fixed inset-y-0 left-0 z-40 w-72 border-r flex flex-col justify-between p-6 transition-transform duration-300 ease-in-out lg:static lg:translate-x-0
        ${containerClasses}
      ...
      `}>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className={`rounded-xl p-4 border transition-colors ${simpleMode ? 'bg-white border-[#E6DEC9] hover:bg-[#FDFCF7]' : 'bg-slate-900/60 border-slate-900 hover:border-slate-800'}`}>
            <div className="flex items-start gap-3">
              ...
              <div>
                <span className="text-[10px] uppercase tracking-wider text-stone-500 font-extrabold font-mono">Student</span>
                <h3 className={`font-bold text-sm ${simpleMode ? 'text-stone-900' : 'text-slate-200'}`}>Muhammad Imran</h3>
                <code className="text-[11px] text-amber-700 mt-0.5 block font-mono">u3004795 · UEL</code>
              </div>
            </div>
            <div className="mt-3 pt-3 border-t border-stone-200/50 flex gap-2 items-center text-stone-600 text-xs">
              ...
              <div className="truncate">
                <span className="text-[10px] text-stone-400 block">Supervisor</span>
          ...
          <nav className="flex flex-col gap-1">
            <span className={`text-[10px] font-bold uppercase tracking-wider px-3 mb-2 block font-mono ${simpleMode ? 'text-stone-500' : 'text-slate-600'}`}>
              Review Chapters
            </span>
            {menuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  id={`nav-btn-${item.id}`}
                  className={`
                    w-full flex items-center justify-between px-4 py-3 rounded-lg text-sm transition-all duration-200 cursor-pointer
                    ${isActive 
                      ? simpleMode
        ...
        {/* Footer Block */}
        <div className={`pt-6 border-t font-mono text-[11px] ${simpleMode ? 'border-amber-200 text-stone-500' : 'border-slate-900 text-slate-500'}`}>
